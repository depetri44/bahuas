        ...
        paymentMessage: (status) => {
            ...
            if(status === 'Pending'){
                ...
                if(config.paymentGateway === 'stripe'){
                    return `<h2 class="text-danger">${paymentConfig.resultMessage}</h2>`;
                }
                return '<h2 class="text-success">El pago de este pedido está pendiente. Nos pondremos en contacto en breve.</h2>';
            }
            return '<h2 class="text-success">El pago de este pedido está pendiente. Nos pondremos en contacto en breve.</h2>';
        },
        paymentOutcome: (status) => {
            if(status === 'Paid' || status === 'Pending'){
                return '<h5 class="text-warning">Conserve los datos anteriores como referencia de pago</h5>';
            }
            return '';
        },
        toUpper: (value) => {
